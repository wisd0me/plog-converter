...
JUOutput::JUOutput(const ProgramOptions &opt) : IOutput(opt, "junit")
{

}

JUOutput::~JUOutput() = default;

void JUOutput::Start()
{
}

void JUOutput::Write(const Warning& msg)
{
...
static void _Write(std::basic_ostream<char>& m_ostream, const Warning& msg)
{
  ...
  if (msg.HasMISRA())
    m_ostream << "      MISRA: " << msg.GetMISRAStringWithLanguagePrefix() << std::endl;
